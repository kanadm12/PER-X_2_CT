"""
Script to vertically flip all DRR images in patient folders.
This script will process all PNG files (DRR images) found in subdirectories
of the drr_patient_data folder, save them with '_flipped' suffix, and delete originals.
"""

import os
from pathlib import Path
from PIL import Image
from tqdm import tqdm

def flip_drr_images(root_dir):
    """
    Vertically flip all DRR PNG images in patient folders.
    Saves flipped images with '_flipped' suffix and deletes the originals.
    
    Args:
        root_dir (str): Root directory containing patient folders (e.g., /workspace/drr_patient_data/)
    """
    root_path = Path(root_dir)
    
    if not root_path.exists():
        print(f"Error: Directory {root_dir} does not exist!")
        return
    
    # Find all PNG files in subdirectories (exclude already flipped ones)
    all_png_files = list(root_path.rglob("*.png"))
    png_files = [f for f in all_png_files if '_flipped' not in f.stem]
    
    if not png_files:
        print(f"No PNG files found in {root_dir}")
        return
    
    print(f"Found {len(png_files)} PNG files to process")
    
    processed = 0
    errors = 0
    deleted = 0
    
    for png_file in tqdm(png_files, desc="Flipping DRR images"):
        try:
            # Read the image
            img = Image.open(png_file)
            
            # Vertically flip the image (flip top-to-bottom)
            flipped_img = img.transpose(Image.FLIP_TOP_BOTTOM)
            
            # Save with _flipped suffix
            output_path = png_file.with_name(png_file.stem + '_flipped.png')
            flipped_img.save(output_path)
            processed += 1
            
            # Close images to free memory
            img.close()
            flipped_img.close()
            
            # Delete the original file
            os.remove(png_file)
            deleted += 1
            
        except Exception as e:
            print(f"\nError processing {png_file}: {e}")
            errors += 1
    
    print(f"\n{'='*60}")
    print(f"Processing complete!")
    print(f"Successfully flipped: {processed} images")
    print(f"Original files deleted: {deleted} images")
    if errors > 0:
        print(f"Errors encountered: {errors} images")
    print(f"{'='*60}")


def main():
    # Configuration
    ROOT_DIR = "/workspace/drr_patient_data"  # Change this path if needed
    
    print("="*60)
    print("DRR Image Vertical Flip Script")
    print("="*60)
    print(f"Root directory: {ROOT_DIR}")
    print("Action: Flip images, save with '_flipped' suffix, delete originals")
    print()
    
    flip_drr_images(root_dir=ROOT_DIR)


if __name__ == "__main__":
    main()
